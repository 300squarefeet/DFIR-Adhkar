/**
 * Renders the 10 most-recently-updated cases from /v1/cases/recent.
...
export function RecentCasesPanel() {
  ...
  useEffect(() => {
    if (!apiCall) return;
    let cancelled = false;
    apiCall<RecentCase[]>("/v1/cases/recent?limit=10")
      .then((r) => {
  ...
  return (
    <article className="mb-4 rounded border border-md-sys-color-outline-variant bg-md-sys-color-surface p-3">
      <header className="mb-2 flex items-center gap-2 text-sm">
        <h2 className="font-medium">Recently active cases</h2>
        <span className="text-xs text-md-sys-color-on-surface-variant">sorted by last update</span>
      </header>
      {error !== null ? (
        ...
        <p className="text-xs text-md-sys-color-on-surface-variant">No cases yet.</p>
      ) : (
